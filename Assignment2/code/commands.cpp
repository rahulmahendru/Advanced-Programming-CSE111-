...
void fn_make(inode_state &state, const wordvec &words)
{
   ...
   if (ptr == check.end())
   {
      inode_ptr newFile = state.getCwd()->getContents(file_type::DIRECTORY_TYPE)->mkfile(words[1]);
      if (words.size() > 2)
      {
         newFile->getContents(file_type::PLAIN_TYPE)->writefile(words);
      }
   }
   else
   {
      if (ptr->second->getFileType() == file_type::PLAIN_TYPE)
      {
         ptr->second->getContents(file_type::PLAIN_TYPE)->writefile(words);
      }
      else
      {
         throw command_error("Directory exists");
      }
   }
}
...
void fn_mkdir(inode_state &state, const wordvec &words)
{
   ...
   if (ptr != check.end())
   {
      throw command_error("Directory already exists");
   }
   else {
      state.getCwd()->getContents(file_type::DIRECTORY_TYPE)->mkdir(words[1]);
   }
...
void fn_pwd(inode_state &state, const wordvec &words)
{
   DEBUGF('c', state);
   DEBUGF('c', words);
   inode_ptr current = state.getCwd();
   string path = state.getPath(current);
   cout << path << "\n" ;
}
